...
谷歌翻译把'许愿伊莉雅和美游'翻译成'vow Yi Liya US tour',我得写一个英文的readme了.
英文readme已经写到吐了.日文readme?别催,学到あ了.
...
直接在fgo\_py.py后追加代码,或者把fgo\_py.py作为一个模块导入后调用里面的函数  
建议在命令行中`import fgo_py`然后交互执行以免反复初始化加载资源  
示例在example.py中  
**每次开肝前都请确认编队与技能的设定是否正确**  

+ 在点击列表中第一个关卡就能进入选择助战界面的界面启动`main`函数来清空体力条,该函数有至少三个参数:要吃的苹果次数(默认0),苹果种类(0金1银2铜3彩,默认0),战斗函数(默认`oneBattle`).之后是要向战斗函数中传递的各项参数,战斗函数应负责从进入战斗后尚未选卡的界面到出现羁绊点结算或队伍全灭提示的部分,并应返回战斗是否成功  
+ 在进入战斗后尚未选卡的界面启动`oneBattle`函数来完成这一战斗,若传入一个有三个`int`的`list`就会在各stage优先攻击左起自1开始编号的敌人  
+ 如果要三回合速刷,请自己写一个战斗函数  

这个脚本会自动放技能([可以设定](#你可以修改skillInfo数组),默认不用技能),自动放宝具([可以设定](#你可以修改houguInfo数组),默认充能满就直接放宝具),自动选卡(优先三色chain,否则优先红卡).合理设定后实战7-12回合能够刷完无限池终本  
...
## 你需要修改slnPath,adbPath和hWnd  
`slnPath`是本解决方案的绝对位置,方便你把代码和其他资源分开放  
`adbpath`要根据你的设备连接方式进行修改  
`hWnd`为显示游戏画面所在的窗口句柄,对于多数模拟器而言,一个主窗口(overlapped window)下就只有一个用于显示游戏画面的子窗口(child window),这时你只需要把hWnd所在定义行中的字符串改为模拟器的主窗口标题就可以了.如果你不知道win32怎么用,就用下面那行被注释掉的代码,启动脚本后一秒钟内把光标挪到模拟器窗口正上方直到脚本开始动作.  
## 你可以修改skillInfo和houguInfo  
建议在`setInfo`函数中修改,以实现"快速换装"效果   
...
如果你用的是模拟器,请使用本程序的`screenShot`函数或`Check().save()`来截图而不要用模拟器自带的截图,更不要用<kbd>prtSc</kbd>键或其他工具来截图,这可能会导致识别错误  
如果你用的是手机,这些函数也可以大大方便你的操作  
`screenShot`函数已不在主程序中使用,你需要手动解除注释  
使用`Check().save()`函数时,建议将显示画面和安卓系统画面都调整至1080p,以避免缩放带来的误差  
使用`screenShot`函数时,请在手机的/storge/emulated/0/目录下新建一个名为adbtemp的文件夹  
助战截图的文件名中若有下划线'\_',且下划线之前的部分为km,ml,cba,则代表好友就挂的是孔明,梅林,十八岁美少女,这会影响助战技能的使用,参见`chooseFriend`函数具体实现  
## 如何将此脚本应用于真机  
v2.7.x及之前版本能在任意支持python和adb的系统上用于任何1080p的Android设备,v2.7版本的代码现在也是可以用的尽管可能不是那么稳定功能也差得远,x2.8.0及之后的版本仅仅由于截图方式由adb改为win32api而只能在windows系统使用,如果使用真机,要么把手机屏幕找一个投屏软件投到屏幕上(建议,我就是这么做的),要么把`Check.__init__()`中的`self.im`初始化为由自`screenShot`产生的图片读取得来的数据,注意,由于当前版本是基于新截图方式进行的优化,进行此替换虽然不会导致错误,但是会使运行效率比v2.7.x还要低  
使用真机时,你还需要以下操作:  
投屏软件画质和分辨率尽可能开高  
